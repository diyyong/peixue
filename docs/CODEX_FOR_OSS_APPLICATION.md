...
**仍建议先完成首个 Release 再提交。** 公共仓库 [diyyong/peixue](https://github.com/diyyong/peixue) 已经提供可构建前端源码、依赖锁文件、第三方许可证、通过的 CI 和安全文档，但还没有 Release 与持续维护/采用证据。下一步是：

1. 轮换真实凭据，并用 Docker 从空数据库完成一次端到端验收
2. 发布至少一个带安装和升级说明的 Release
3. 积累一段真实维护记录和可验证的使用反馈
4. 再用下面的证据写申请
